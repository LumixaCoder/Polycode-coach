...
NAME_TO_MODULE = {}
NAME_TO_MODULE.update(ASSIGN_MAP)
NAME_TO_MODULE.update(FUNC_MAP)
NAME_TO_MODULE.update(CLASS_MAP)

# Some additional names like BADGES uses internal helpers that are in same module, we don't need external
# But we also need to handle names that are Assign keys but also used elsewhere like DATASET_FILES etc.
# Build reverse for each module: defined names set
module_defs = collections.defaultdict(set)
for name, mod in NAME_TO_MODULE.items():
    module_defs[mod].add(name)

# Also need to consider that some modules define multiple names with same key like _WORKER_SRC duplicated -> still same
# Now for each generated file, parse its AST and find Name loads that are not defined locally nor builtin nor local
import pathlib

BUILTINS = set(dir(__builtins__)) if isinstance(__builtins__, dict) else set(dir(__builtins__))
# Add common builtins and keywords that are not in NAME_TO_MODULE
STDLIB_NAMES = {"print", "len", "range", "type", "str", "int", "float", "bool", "dict", "list", "set", "tuple", "open", "enumerate", "zip", "sorted", "max", "min", "sum", "any", "all", "map", "filter", "reversed", "abs", "round", "pow", "input", "isinstance", "issubclass", "hasattr", "getattr", "setattr", "callable", "Exception", "ValueError", "KeyError", "FileNotFoundError", "OSError", "json", "os", "sys", "time", "pathlib", "Path", "date", "datetime", "timedelta", "tk", "re", "ast", "random", "subprocess", "tempfile", "base64", "copy", "_copy", "importlib", "messagebox", "argparse"}

for mod_path, defs in module_defs.items():
    ...
    try:
        ...
        print(f"SYNTAX {mod_path}: {e}")
        continue
    # collect all defined names in this file (including those we mapped plus local vars? For simplicity, only top-level defs)
    defined = set(defs)
    # also collect top-level assigns that may not be in map but defined (like internal locals?) but we only care about cross-module
    # Find all Name nodes that are Load
    needed = collections.defaultdict(set)  # name -> set of function/class where used? For now per file
    # For each function/class node, scan its body
    for node in ast.walk(tree):
        if isinstance(node, ast.Name) and isinstance(node.ctx, ast.Load):
            name = node.id
            if name in defined:
                continue
            if name in BUILTINS or name in STDLIB_NAMES:
                continue
            # if name starts with _ and is not in map, maybe it's internal like _copy?
            # Check if name is defined in some other module
            if name in NAME_TO_MODULE:
                needed[name].add(NAME_TO_MODULE[name])
            else:
                # Possibly name is local variable or builtin we missed, or stdlib module alias
                # ignore if name is lowercase builtin like _WORKER_SRC already handled etc.
                # For debugging, print unknown
                pass
    if needed:
        # reverse: for each other module, list names
        by_target = collections.defaultdict(list)
        for name, targets in needed.items():
            for tgt in targets:
                if tgt != mod_path:
                    by_target[tgt].append(name)
        print(f"\n{mod_path} needs:")
        for tgt, names in sorted(by_target.items()):
            uniq = sorted(set(names))
            print(f"  from {tgt} import {', '.join(uniq)}  -> used in {mod_path}")

# Also check for undefined due to missing local var detection: Use simple heuristic
